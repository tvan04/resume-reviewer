...
jest.mock("firebase/storage", () => ({
  ...
  uploadBytesResumable: jest.fn(() => ({
    on: jest.fn((event, progressFn, _, successFn) => {
...
describe("UploadScreen", () => {
  ...
  it("handles PDF upload successfully", async () => {
    ...
    await act(async () => {
      fireEvent.click(screen.getByLabelText("Reviewer One"));
    });
    ...
    await waitFor(() => {
      expect(screen.getByText(/Select File\(s\)|Upload Files/i)).toBeInTheDocument();
    });
    ...
    await waitFor(() => expect(mockOnUpload).toHaveBeenCalled(), { timeout: 3000 });
  ...
  it("navigates back to student dashboard", async () => {
    await act(async () => {
      render(
        <MemoryRouter>
          <UploadScreen user={mockUser} onUpload={mockOnUpload} />
